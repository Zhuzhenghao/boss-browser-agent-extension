...
export function getTaskStageSummary(task, toolTimeline = []) {
  ...
  if (task?.status === 'error') {
    return task?.error || '这轮巡检执行异常，请检查失败候选人后再决定是否重试。';
  }
...
export function getToolEventFingerprint(event) {
  ...
  const candidateName = event.candidateName ?? '';
...
export function areToolEventsEqual(prevEvent, nextEvent) {
  return getToolEventFingerprint(prevEvent) === getToolEventFingerprint(nextEvent);
}
...
export function dedupeToolEvents(events = []) {
  ...
  return list.filter(event => {
    const fingerprint = getToolEventFingerprint(event);
