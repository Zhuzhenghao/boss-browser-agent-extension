...
function shouldShowEventSummary(event) {
  // 模型输出始终显示 summary（model_response / model_reasoning / model_final_output）
  if (
    event.toolName === 'model_reasoning' ||
    event.toolName === 'model_response' ||
    event.toolName === 'model_final_output'
  ) {
    return true;
  }
  // 其他事件隐藏 summary，只显示工具名称和状态
  return false;
}
