...
export async function openCandidateChat({
  ...
  const verifyCurrentChat = async () => {
    const result = await browserAgent.aiQuery(
      `只观察当前主内容区域，不要操作页面。请判断当前是否已经进入名字为“${name}”的候选人聊天会话。只返回 JSON：{"ready":true|false,"reason":"一句中文依据"}。只有当主视图里能确认当前聊天对象就是 ${name}，并且同时看到右侧主聊天区域的聊天消息区、位于消息列表下方和聊天操作按钮下方的白色空白编辑区，以及该编辑区右下角的”发送”按钮时，才返回 true。不要因为只看到了部分聊天操作按钮、部分输入区边缘或其他模糊布局就返回 true。`,
    );
