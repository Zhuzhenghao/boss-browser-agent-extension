...
import { ensureDir, getRuntimePath, migrateLegacyFile } from '../../server/runtime-paths.js';

const DATA_DIR = getRuntimePath('screening-data');
...
function ensureDbPath() {
  if (!fs.existsSync(DB_PATH)) {
    migrateLegacyFile(path.join('screening-data', 'screening.sqlite'));
  }
}
...
function initializeSchema(db) {
  db.exec(`
    ...
    CREATE TABLE IF NOT EXISTS model_configs (
      ...
      chat_model_family TEXT NOT NULL DEFAULT '',
      text_model_name TEXT NOT NULL DEFAULT 'qwen-plus',
      gateway_api_key TEXT NOT NULL DEFAULT '',
  ...
  ensureColumn(db, 'model_configs', 'chat_model_family', 'TEXT NOT NULL DEFAULT \'\'');
  db.exec(`
