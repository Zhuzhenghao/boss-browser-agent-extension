...
export function recordFatalBridgeError(state, error) {
  ...
  const currentTaskId = String(
    state?.unreadTaskState?.task?.taskId || state?.unreadTaskState?.taskId || '',
  ).trim();
  ...
  if (currentTaskId) {
    void readScreeningTask(currentTaskId)
      .then(task => {
        if (!task) return null;
        return persistScreeningTask({
          ...task,
          status: 'failed',
...
export async function handleScreenUnreadStart(req, res, state) {
  ...
  child.on('message', message => {
    ...
    if (message.type === 'task-failed') {
      ...
      mergeUnreadTaskState(state, {
        running: false,
        status: message.data?.aborted ? 'stopped' : 'failed',
        finishedAt: new Date().toISOString(),
  ...
  child.on('exit', async code => {
    ...
    const persistedTask = await readScreeningTask(taskId).catch(() => null);
    const persistedStatus = String(persistedTask?.status || '').trim();
    const hasUnfinishedCandidates = Array.isArray(persistedTask?.candidates)
      && persistedTask.candidates.some(candidate =>
        ['queued', 'running', 'failed'].includes(candidate.status),
      );

    if (persistedTask && (persistedStatus === 'running' || persistedStatus === 'queued' || hasUnfinishedCandidates)) {
      const unexpectedExitMessage = code === 0
        ? '任务 worker 异常退出，主流程尚未真正开始或未正常收尾。可继续执行未完成候选人。'
        : `任务 worker 异常退出 (${code ?? 'unknown'})，可继续执行未完成候选人。`;

      const repairedTask = await persistScreeningTask({
        ...persistedTask,
        status: 'failed',
        finishedAt: new Date().toISOString(),
        summary: unexpectedExitMessage,
        error: unexpectedExitMessage,
      }).catch(() => null);
      ...
      mergeUnreadTaskState(state, {
        ...
        finishedAt: new Date().toISOString(),
        taskId,
        task: repairedTask || persistedTask,
        error: unexpectedExitMessage,
      });
      return;
    }
